...
language sql volatile security definer set search_path to 'public', 'pg_temp'
as $$
  with candidates as (
    select q.id from public.prospecting_outbound_queue q
     where q.organization_id = p_org and q.status = 'pending' and q.scheduled_for <= now()
  ...
  returning q.*;
$$;
revoke execute on function public.fn_claim_prospecting_outbound(uuid, integer) from public, anon, authenticated;
grant execute on function public.fn_claim_prospecting_outbound(uuid, integer) to service_role;
  ...
  if new.direction = 'inbound' then
    update public.prospecting_outbound_queue
     ...
     where organization_id = new.organization_id and contact_id = new.contact_id
       and kind = 'followup' and status in ('pending', 'processing');
    update public.crm_leads
       set custom_fields = jsonb_set(
           ...
           last_activity_at = coalesce(new.sent_at, now()), updated_at = now()
...
create trigger trg_cancel_prospecting_followup_on_inbound after insert on public.messages
...
begin
  if new.is_blocked and not old.is_blocked and exists (
    ...
    update public.contacts
       set tags = array(select distinct x from unnest(coalesce(tags, '{}'::text[]) || array['nao-contatar']) x), updated_at = now()
    ...
    update public.crm_leads
       set tags = array(select distinct x from unnest(coalesce(tags, '{}'::text[]) || array['nao-contatar']) x),
           custom_fields = jsonb_set(jsonb_set(coalesce(custom_fields, '{}'::jsonb), '{opt_out}', 'true'::jsonb, true), '{next_followup_at}', 'null'::jsonb, true),
           updated_at = now()
    ...
    update public.prospecting_outbound_queue
     ...
     where organization_id = new.organization_id and contact_id = new.id and status in ('pending', 'processing');
...
create trigger trg_apply_prospecting_opt_out after update of is_blocked on public.contacts
for each row execute function public.fn_apply_prospecting_opt_out();
revoke execute on function public.fn_cancel_prospecting_followup_on_inbound() from public, anon, authenticated;
revoke execute on function public.fn_apply_prospecting_opt_out() from public, anon, authenticated;
grant execute on function public.fn_cancel_prospecting_followup_on_inbound(), public.fn_apply_prospecting_opt_out() to service_role;
...
-- ---- 20260911184300_0176_orders_customer_history_rpc ----
-- EPIC-21 (Athos): RPC de historico de compras por cliente para a Sarah montar
-- campanhas de recompra / reativacao. SECURITY INVOKER + filtro manual de
-- `organization_id` (padrao das leituras do projeto - service-role atravessa
-- RLS, e o filtro e o que isola). STABLE: `now()` vem como argumento `p_now`
-- (default now()), assim o planner trata como leitura pura. Apendice e
-- byte-a-byte identico a migration; uma divergencia aqui quebraria clones
-- que rodaram a migration mas estao re-aplicando o baseline (update.sh sem
-- ON_ERROR_STOP = pula em silencio).
...
-- ---- 20260924183000_0181_food_athos_product_map ----
