...
-- ---- emit_event signature drift (migration 0094) ----
-- DRIFT observado em produção: o banco tem versão antiga da função
-- `public.emit_event(p_entity_kind, p_event_type, p_metadata, p_organization_id,
-- p_payload)` — 5 parâmetros, sem `p_entity_id`. O repo (canônico desde
-- migration 0093) define 6 parâmetros incluindo `p_entity_id`. PostgREST
-- recusa o RPC por divergência de assinatura → webhook WAHA → 503 → WAHA
-- retry-loop no segundo turno da Sarah.
--
-- Reconciliação idempotente: re-cria `emit_event` com a assinatura canônica
-- (CREATE OR REPLACE não quebra quem chama porque tem o mesmo name). NÃO
-- altera grants (a função permanece SECURITY DEFINER com grants existentes).
-- `NOTIFY pgrst, 'reload schema'` força PostgREST a recarregar o cache
-- imediatamente; sem isto o cache stale pode persistir até a próxima
-- reconexão do PostgREST (≈10min no default).
