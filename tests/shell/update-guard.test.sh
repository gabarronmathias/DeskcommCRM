...
cat > "$PROJ/.env" <<ENV
APP_IMAGE=ghcr.io/melgarafael/gm-crm:latest
...
check ".env aponta para a imagem da versão instalada" grep -q '^APP_IMAGE=ghcr.io/melgarafael/gm-crm:1.1.0$' .env
...
check "o worker é pinado na MESMA versão do app" \
  grep -q '^WORKER_IMAGE=ghcr.io/melgarafael/gm-crm-worker:1.1.0$' .env
check "o scheduler é pinado na MESMA versão do app" \
  grep -q '^SCHEDULER_IMAGE=ghcr.io/melgarafael/gm-crm-scheduler:1.1.0$' .env
...
check "a imagem do .env continua intacta" grep -q '^APP_IMAGE=ghcr.io/melgarafael/gm-crm:latest$' .env
...
check "NÃO reescreveu a imagem do .env" grep -q '^APP_IMAGE=ghcr.io/melgarafael/gm-crm:latest$' .env
