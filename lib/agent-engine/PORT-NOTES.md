# agent-engine — notas do porte Vendaval → G&M CRM (fusão)
...
| Vendaval (antigo) | G&M CRM (canônico) |
|---|---|
...
- AI SDK do repo: `ai` ^6 + `@ai-sdk/anthropic` ^3 (NÃO v7/v4 do Vendaval). Espelhar o uso de
  `generateText`/usage que `lib/ai/runtime/agent.ts` do G&M CRM já faz.
...
- Canvas/console do Vendaval: descartados (UI do G&M CRM substitui).
