...
async function patchQueue(db: SupabaseClient, row: QueueRow, patch: Record<string, unknown>) {
  const { error } = await db.from("prospecting_outbound_queue").update({ ...patch, updated_at: new Date().toISOString() })
...
async function reconcileExisting(db: SupabaseClient, row: QueueRow) {
  const { data } = await db.from("messages").select("id, status, error_code")
...
async function onSent(db: SupabaseClient, row: QueueRow, messageId: string): Promise<DispatchResult> {
  const now = new Date();
  await patchQueue(db, row, { status: "sent", sent_at: now.toISOString(), crm_message_id: messageId, error_code: null, error_message: null });

  const { data: lead } = await db.from("crm_leads").select("custom_fields")
  ...
  const fields = {
    ...
    ...(row.kind === "opening" && !current.first_outbound_at ? { first_outbound_at: now.toISOString() } : {}),
    last_outbound_at: now.toISOString(),
    next_followup_at: row.kind === "opening" ? new Date(now.getTime() + 48 * 60 * 60_000).toISOString() : null,
  };
  await db.from("crm_leads").update({ custom_fields: fields, last_activity_at: now.toISOString() })
    .eq("organization_id", row.organization_id).eq("id", row.lead_id);

  if (row.kind === "opening") {
    const { data: pipelineRef } = await db.from("crm_leads").select("pipeline_id")
      .eq("organization_id", row.organization_id).eq("id", row.lead_id).maybeSingle();
    if (pipelineRef) {
      const { data: stage } = await db.from("crm_stages").select("id")
        .eq("organization_id", row.organization_id).eq("pipeline_id", pipelineRef.pipeline_id)
        .eq("name", SARAH_STAGE_NAME).eq("is_archived", false).maybeSingle();
      if (stage) {
        await db.from("crm_leads").update({ stage_id: stage.id, updated_at: now.toISOString() })
          .eq("organization_id", row.organization_id).eq("id", row.lead_id);
      }
    }
...
export async function dispatchQueueRow(db: SupabaseClient, row: QueueRow): Promise<DispatchResult> {
  const [{ data: contact }, { data: conversation }, { data: session }] = await Promise.all([
    db.from("contacts").select("is_blocked, force_human, tags").eq("organization_id", row.organization_id).eq("id", row.contact_id).maybeSingle(),
    db.from("conversations").select("assigned_to_user_id, last_inbound_at, bot_silenced_until").eq("organization_id", row.organization_id).eq("id", row.conversation_id).maybeSingle(),
    db.from("channel_sessions").select("status").eq("organization_id", row.organization_id).eq("id", row.channel_session_id).maybeSingle(),
  ]);
  if (!contact || !conversation || !session) return cancel(db, row, "tenant_resource_missing");
  if (contact.is_blocked || (contact.tags ?? []).includes("nao-contatar")) return cancel(db, row, "opt_out");
  if (contact.force_human || conversation.assigned_to_user_id || conversation.bot_silenced_until) return cancel(db, row, "human_takeover");
  ...
  if (existing?.status === "queued" || existing?.status === "sending") return hold(db, row, "message_still_queued");
  if (existing?.status === "failed" && row.attempts >= row.max_attempts) {
    await patchQueue(db, row, { status: "failed", error_code: existing.error_code ?? "message_failed" });
    return { queueId: row.id, outcome: "failed", reason: existing.error_code ?? "message_failed" };
  }

  try {
    const message = await sendMessageHandler(db, {
      organization_id: row.organization_id,
    ...
    if (message.status === "failed") {
      await patchQueue(db, row, { status: "failed", crm_message_id: message.id, error_code: message.error_code ?? "message_failed", error_message: message.error_message?.slice(0, 240) ?? null });
      return { queueId: row.id, outcome: "failed", messageId: message.id, reason: message.error_code ?? "message_failed" };
    }
    await patchQueue(db, row, { status: "pending", claimed_at: null, crm_message_id: message.id, scheduled_for: new Date(Date.now() + 5 * 60_000).toISOString(), error_code: "message_queued" });
    ...
    if (row.attempts >= row.max_attempts) {
      await patchQueue(db, row, { status: "failed", error_code: "dispatch_error", error_message: detail });
...
export async function claimOne(db: SupabaseClient, organizationId: string): Promise<QueueRow | null> {
  const { data, error } = await db.rpc("fn_claim_prospecting_outbound", { p_org: organizationId, p_limit: 1 });
