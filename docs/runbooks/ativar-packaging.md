...
```console
$ ghcr_status gm-crm 1.2.1        → 200
$ ghcr_status gm-crm stable       → 404   (o canal ainda não existe)
$ ghcr_status gm-crm-worker latest   → 403   (o pacote ainda não existe)
```
...
(`gm-crm-worker`, `gm-crm-scheduler`) publicando `main` e `latest` neles.

**Verificação:**

```bash
gh run list --workflow=publish-image.yml --limit 3    # o run da main ficou verde?
ghcr_status gm-crm-worker latest                    # esperado agora: 403 (existe, privado)
```
...
Para cada um de `gm-crm-worker` e `gm-crm-scheduler`:
...
```bash
gh api -X PATCH repos/melgarafael/G&M CRM/branches/main/protection/required_status_checks \
...
> nome do check passou a ser `build-and-push (gm-crm, …)` — exigir cada um pelo nome faria
