...
2. **PR e merge na `main`.** `publish-image.yml` dispara em push na `main` (ou
   tag `v*`) e publica **três** imagens — `gm-crm`, `gm-crm-worker` e
   `gm-crm-scheduler` — sempre na mesma versão. O build pesado roda nos
...
```bash
APP_IMAGE=gm-crm-app:local docker compose \
  -f docker-compose.prod.yml -f docker-compose.build.yml --env-file .env build app

APP_IMAGE=gm-crm-app:local APP_PULL_POLICY=never docker compose \
