# Fusão Vendaval → G&M CRM — briefing de execução (Terminal B)
...
**Um repositório só: o G&M CRM.** Fusão total do back-end do Vendaval (bom, mas nunca ligado à
realidade) dentro do G&M CRM (Next.js + TS + Supabase + WAHA + AI SDK — CRM real com chat ao vivo,
...
G&M CRM como borda — o contrato exato está em `~/vendaval/docs/specs/edge-contract.md` (referencia
arquivos e linhas reais do G&M CRM). Como agora é UM repo, a maquinaria de "dois sistemas" colapsa em
chamadas internas.

**Encaixes verificados no código real do G&M CRM:**
...
- Entrada: WAHA → `event_log` (RPC `emit_event`). O G&M CRM tem um dispatcher nativo (`dispatchAgents`).
...
- Modelo de dados do G&M CRM é o CANÔNICO: `organizations`/`contacts`/`conversations`/`messages`/
...
→ resposta aparece no CHAT do G&M CRM. *Prova:* você manda um WhatsApp de verdade a um número conectado e VÊ o
...
**Fase 2 — continuidade + config real.** Memória/RAG/follow-up no turno (usando histórico real do G&M CRM) +
...
- Do G&M CRM: siga o `CLAUDE.md`/`AGENTS.md`/MANIFEST do repo (migrations, RLS, LGPD nativa, multi-tenant).
