...
# 🛠️ G&M CRM — The open-source AI Sales OS for WhatsApp
...
>
> G&M CRM is developed in **partnership with HostGator**: the [`hostgator-setup-kit/`](hostgator-setup-kit/)
...
*"install G&M CRM for me"*. It reads the kit's [`CLAUDE.md`](hostgator-setup-kit/CLAUDE.md)
...
**G&M CRM** is the sales operating system by Gabarron & Mathias: one unified workspace where your entire WhatsApp operation — sales, support and follow-through — is run by people and AI agents together. Multi-tenant, LGPD by-design, built for Brazilian operations that sell by conversation.
...
Open an [issue](https://github.com/gabarronmathias/gm-crm/issues/new/choose) — the template asks for what we need (environment, `/api/v1/health`, steps). Running `bash hostgator-setup-kit/healthcheck.sh` and pasting the output helps a lot.

For **security vulnerabilities**, **do NOT open a public issue** — use [private vulnerability reporting](https://github.com/gabarronmathias/gm-crm/security/advisories/new). Details in [`SECURITY.md`](SECURITY.md).
...
- **Public MCP** — CRM capabilities exposed to the agent ecosystem: plug in any agent and it operates G&M.
...
- **Discussions:** [GitHub Discussions](https://github.com/gabarronmathias/gm-crm/discussions)
...
- The community that took G&M from e-commerce to clinics, real estate, info-products and beyond — you defined what this project is.
